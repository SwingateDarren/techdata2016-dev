...
class tp_eventbrite_importer{
    public $EventFilterDates = array();
    public $EventVendors = array();
    public $existingEvents = array();
    public $foundInImport = array();
    public $dataSets = Array();

    function start(){
        $this->EventFilterDates = $this->load_term_tree('date_filter');
        $this->EventVendors = $this->load_term_tree('vendors_term');
        // get the local events from your database
        $this->retieve_local_events();
        // get the eventbrite list of events
        $this->DataSets['microsoft'] = $this->import_from_eventbrite('14978389890'); // microsoft
        // process this data and list which were still found
        $this->process_import_against_local($this->DataSets['microsoft'], 'Microsoft');
        // clean up the list
        $this->clean_up_local_data();
        error_log("Finished - complete");

    }
    function load_term_tree($vocabulary_name){
        if ($vocabulary = taxonomy_vocabulary_machine_name_load($vocabulary_name)) {
            return $this->formatTree(taxonomy_get_tree($vocabulary->vid),"0");
        }
        return Array();
    }
    function formatTree($list,$parentID){
        ...
        foreach($list as $index=>$node){
            if($node->parents[0] === $parentID){
                if($node->depth==0){
                    $node->children = $this->formatTree($list, $node->tid);
                }
                $level[count($level)] = $node;
            }
        }
        return $level;
    }

    function import_from_eventbrite($organiser){
        $client = new HttpClient('FPMOK2VR6L24NIH4BH5Z');
        return $client->get_event_search(Array('organizer.id='.$organiser));
    }
    function process_import_against_local($data, $vendorName){
        foreach($data['events'] as $index => $record){
            $node = $this->event_exists($record,$this->existingEvents);
            if($node === -1){
                $this->add_event($record , $vendorName);
            } else {
                $this->foundInImport[count($this->foundInImport)] = $node->nid;
                $this->edit_event($record, $node, $vendorName );
            }
        }
        error_log("Finished - processing");
    }
    function clean_up_local_data(){
        error_log("Start - cleanup");
        foreach ($this->existingEvents as $index => $node) {
            if(!in_array($node->nid, $this->foundInImport)){
                node_delete($node->nid);
            }
        }
        error_log("Finished - cleanup");
    }   


    function retieve_local_events(){
        $result = db_query("SELECT nid, title FROM node WHERE type = 'event'");
        $this->existingEvents = array();
        foreach ($result as $row) {
            $this->existingEvents[$row->nid] = node_load($row->nid);
        }
    }


    function event_exists($record,$list){
        foreach($list as $nid => $existingRecord){
            if($existingRecord->field_eventbrite_id['und'][0]['value'] == $record['id']){
                return $existingRecord;
            }
        }
        return -1;
    }
    function add_event($record, $vendorName ){
        $nodeType = 'event';
        $node = new stdClass();
        $node->type = $nodeType;
    ...
    function edit_event($record,$node, $vendorName ){
        error_log("Edittign record");
        $node->title = (str_replace(Array('\u2019','\u2018'),"'",$record['name']['text']));
        $node->field_thumbnail_url['und'][0]['value'] = $record['logo']['url'];
        $node->field_url['und'][0]['value'] = $record['url'];
        $node->field_eventbrite_id['und'][0]['value'] = $record['id'];
        $node->field_date_of_event['und'][0]['value'] = $record['start']['local'];
        $node->promote = 0; // Display on front page ? 1 : 0
        $node->sticky = 0;  // Display top of page ? 1 : 0
        $node->format = 2;  // 1:Filtered HTML, 2: Full HTML
        $node->status = 1;  // Published ? 1 : 0
        $year = date("Y", strtotime($record['start']['local']));
        $month = date("M", strtotime($record['start']['local']));
        $termId = $this->find_date_term($year,strtolower($month));
        if($termId!=-1){
            $node->field_filterable_date['und'][0]['tid'] = $termId;
        }
        $termId = $this->find_vendor($vendorName);
        if($termId!=-1){
            $node->field_vendor['und'][0]['tid'] = $termId;
        }
    ...
    function find_date_term($year,$month){
        foreach($this->EventFilterDates as $index=> $YearRecord){
            if($YearRecord->name === "$year"){
                foreach($YearRecord->children as $index=> $MonthRecord){
                    if(substr(strtolower($MonthRecord->name),0,3) === "$month"){
                        return $MonthRecord->tid;
                    }
                }
            }
        }
    ...
    function find_vendor($name){
        foreach($this->EventVendors as $index=> $vendor){
            if($vendor->name === "$name"){
                return $vendor->tid;
            }
        }
